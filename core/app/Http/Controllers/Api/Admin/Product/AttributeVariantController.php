...
use App\Models\Product\AttributeVariant as RecordModel;
...
class AttributeVariantController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $responseData['records'] = $responseData['records']->map(function($record){
            $record['variants'] = \App\Models\Product\Variant::select(['id','name'])->whereIn('id',$record['variants'])->get() ;
            return $record;
        });
    ...
    public function create(Request $request){
        $record = RecordModel::where('variants',$request->variants)->first() ;
        if( $record ){
            return response([
                'record' => $record ,
                'ok' => false ,
    ...
    public function read($id)
    {
        ...
        if ($record) {
            return response()->json([
                'record' => $record,
                'variants' => $record->variantCombination()->select(['id','name'])->get() ,
                'ok' => true ,
