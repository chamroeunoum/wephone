...
use App\Models\Product\Attribute as RecordModel ;
...
class AttributeController extends Controller
{
    ...
    public function compact(Request $request){
        ...
        $responseData['records'] = $builder->get()->map(function($attribute){
            $attribute['variants'] = \App\Models\Product\Variant::select(['id','name'])->where('attribute_id',$attribute['id'])->get();
            return $attribute ;
        });
        $responseData['message'] = __("crud.read.success");
        $responseData['ok'] = true ;
    ...
    public function checkName(Request $request){
        if( isset( $request->name ) && $request->name != "" ){
            if( ($records = RecordModel::where('name',$request->name)->get() ) !== null ){
