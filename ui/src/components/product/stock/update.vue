              ...
              <div class="crud-form-panel w-full flex flex-wrap ">
                ...
                >
                  <n-form-item label="ផលិតផល" path="product" class="w-4/5 mr-8" >
                    <n-select v-model:value="record.product_id" :options="productOptions" placeholder="ផលិតផល" filterable />
                  </n-form-item>
                  ...
                  <n-form-item label="បរិមាណ" path="quantity" class="w-4/5 mr-8" >
                    <n-input-number v-model:value="record.quantity" clearable placeholder="បរិមាណ" />
                  </n-form-item>
...
export default {
  ...
  setup(props){
    ...
    function create(){
      if( props.record.product_id <= 0 || props.record.product_id == null ){
        notify.warning({
      ...
      if( props.record.unit_id <= 0 || props.record.unit_id == null ){
        notify.warning({
          'title' : 'ពិនិត្យព័ត៌មាន' ,
    ...
    function getAttributes(){
      console.log( "ME => " ) 
      var selectedVariants = props.record.attributeVariant.variants
      store.dispatch('attribute/compact',{}).then( res => {
        switch( res.status ){
          case 200 : 
            attributes.value = res.data.records.map( (r) => { 
              var selectedVariant = selectedVariants.filter( av => av.attribute_id == r.id )
              return {
              id : r.id ,
              name : r.name ,
              variants : r.variants , 
              variant : ref('') ,
              variant_id : selectedVariant.length ? ref(selectedVariant[0].id) : ref(0)
            }
          })
          break;
        }
